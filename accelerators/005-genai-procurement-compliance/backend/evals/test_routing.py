...
ROUTING_CASES = [
    ("Check compliance of this vendor", "high"),
    ("What gaps exist?", "high"),
    ("What is the risk tier?", "critical"),
    ("Upload attestation document", "medium"),
    ("What are the regulations?", "low"),
    ("Compare vendors", "medium"),
    ("hello", "low"),
]
...
@pytest.mark.anyio
async def test_escalation_on_risk_assessment():
    query = await query_agent.process("Classify the risk of this AI system")
    routing = await router_agent.route(query)

"""Routing evaluation tests for Medi-Cal Eligibility Agent."""
...
class TestPriorityAssignment:
    @pytest.mark.asyncio
    async def test_high_priority_intents(self, query_agent, router_agent):
        """Application status and income verification should be high priority."""
        high_cases = [
            "What is my application status?",
            "I need to verify my income",
        ]
        for msg in high_cases:
            query = await query_agent.process(msg)
            routing = await router_agent.route(query)
            assert routing.priority in ("high", "critical"), f"'{msg}' should be high priority, got {routing.priority}"

    @pytest.mark.asyncio
    async def test_low_priority_intents(self, query_agent, router_agent):
        """General info should be low priority."""
        query = await query_agent.process("What types of Medi-Cal programs exist?")
        routing = await router_agent.route(query)
